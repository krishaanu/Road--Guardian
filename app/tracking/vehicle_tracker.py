...
# ==========================================
# 1. VEHICLE TRAJECTORY MANAGER WRAPPER
# ==========================================
...
class TrackedVehicle:
    ...
    def update_position(self, bbox: Tuple[int, int, int, int], timestamp: float, meters_per_pixel: float = 0.05):
        ...
        if dt > 0 and len(self.history) > 0:
            ...
            smoothed_speed = (0.7 * self.speed_kmh) + (0.3 * inst_speed)
            self.speed_kmh = float(np.clip(smoothed_speed, 0.0, 140.0))
...
# ==========================================
# 3. GLOBAL CROSS-CAMERA TRACKER ENGINE
# ==========================================
...
class CrossCameraTracker:
    ...
    def get_or_create_gvid(self, camera_id: str, local_track_id: int, embedding: Optional[np.ndarray] = None) -> str:
        ...
        # 2. Attempt Cross-Camera Re-ID Match
        ...
        # 3. Create new GVID if no match found
        self.gvid_counter += 1
        new_gvid = f"GV_{camera_id}_{self.gvid_counter}"
        self.local_to_global[scoped_key] = new_gvid
        
        if embedding is not None:
            # Save unit normalized embedding to gallery
